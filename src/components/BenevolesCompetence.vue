...
export default {
  ...
  methods: {
    getBenevolesForm: function (idForm) {
      this.$http
        .get('/competences/FORM/' + idForm + '?ul=' + store.getters.getUtilisateur.structure.id + '&page=0')
        .then(response => {
          this.getEmails(response.data.list)
          for (let page = 1; page !== response.data.pages; page++) {
            this.$http
              .get('/competences/FORM/' + idForm + '?ul=' + store.getters.getUtilisateur.structure.id + '&page=' + page)
              .then(response => {
                this.getEmails(response.data.list)
              })
              .catch(error => {
